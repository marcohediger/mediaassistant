"""
Test: v2.29.5-7 merge features — FTAG-34 to FTAG-50
Covers: user_kept flag, _is_user_kept(), quality_swap protection,
        own_album preservation, donor_albums from Immich/fallback,
...
# ──────────────────────────────────────────────────────────────
# FTAG-34 / FTAG-35: user_kept flag set by keep_file / not_duplicate
# ──────────────────────────────────────────────────────────────
async def test_user_kept_flag_keep():
    """FTAG-34: keep_file sets user_kept=True in IA-02."""
    ...
    ts = int(time.time())
    print("\n── FTAG-34: user_kept via keep_file ──")
    ...
    async with async_session() as session:
        ...
        orig_path = f"/library/photos/2026/__v29_kept_orig_{ts}.jpg"
        dup_path = f"{dup_dir}/__v29_kept_dup_{ts}.jpg"
        ...
        orig = Job(
            filename=f"__v29_kept_orig_{ts}.jpg",
            original_path=orig_path, source_inbox_path="/inbox",
            status="done", target_path=orig_path,
            debug_key=f"V29-KORIG-{ts}", file_hash=f"v29ko_{ts}",
            phash="v29keep00000001",
            step_result={
        ...
        dup = Job(
            filename=f"__v29_kept_dup_{ts}.jpg",
            original_path=dup_path, source_inbox_path="/inbox",
            status="duplicate", target_path=dup_path,
            debug_key=f"V29-KDUP-{ts}", file_hash=f"v29kd_{ts}",
            phash="v29keep00000001",
            step_result={
                "IA-01": {"file_type": "JPEG", "file_size": len(jpeg)},
                "IA-02": {
                    "status": "duplicate", "match_type": "exact",
                    "original_debug_key": f"V29-KORIG-{ts}",
                    "original_path": orig_path,
                    "folder_tags": ["Ferien", "Mallorca", "Ferien Mallorca"],
                },
                "IA-07": {"keywords_written": ["Palmen"]},
            },
        )
        session.add_all([orig, dup])
        await session.commit()
        keep_key = dup.debug_key
        orig_dk = orig.debug_key
        dup_dk = dup.debug_key

    # Simulate keep_file merge + user_kept injection (exact router code path)
    async with async_session() as session:
        ...
        # Minimal merge (actual keep_file does full merge — we test the flag)
        ...
        check("FTAG-34a user_kept in IA-02",
              sr["IA-02"].get("user_kept") is True,
              f"ia02={sr['IA-02']}")
...
async def test_user_kept_flag_not_duplicate():
    """FTAG-35: not_duplicate sets user_kept=True in IA-02."""
    from database import async_session
    from models import Job
    from sqlalchemy.orm.attributes import flag_modified

    ts = int(time.time())
    print("\n── FTAG-35: user_kept via not_duplicate ──")

    async with async_session() as session:
        jpeg = b'\xff\xd8\xff\xe0\x00\x10JFIF\x00' + b'\x00' * 100 + b'\xff\xd9'
        dup_dir = "/library/error/duplicates"
        os.makedirs(dup_dir, exist_ok=True)
        dup_path = f"{dup_dir}/__v29_nd_{ts}.jpg"
        with open(dup_path, 'wb') as f:
            f.write(jpeg)

        job = Job(
            filename=f"__v29_nd_{ts}.jpg",
            original_path=dup_path, source_inbox_path="/inbox",
            status="duplicate", target_path=dup_path,
            debug_key=f"V29-ND-{ts}", file_hash=f"v29nd_{ts}",
            phash="v29nd_000000001",
            step_result={
                "IA-01": {"file_type": "JPEG", "file_size": len(jpeg)},
                "IA-02": {
                    "status": "duplicate", "match_type": "exact",
    ...
    # Simulate not_duplicate logic (exact router code)
    async with async_session() as session:
        from sqlalchemy import select
        ...
        check("FTAG-35a user_kept in skip_result",
              skip_result.get("user_kept") is True,
              f"skip={skip_result}")

        check("FTAG-35b folder_tags preserved in skip_result",
              skip_result.get("folder_tags") == ["Urlaub", "Griechenland", "Urlaub Griechenland"],
              f"ft={skip_result.get('folder_tags')}")

        # Apply to job (in-memory — no reprocess since we test the flag, not the pipeline)
...
# ──────────────────────────────────────────────────────────────
# FTAG-36: _is_user_kept() returns correct values
# ──────────────────────────────────────────────────────────────
async def test_is_user_kept():
    """FTAG-36: _is_user_kept returns True/False correctly."""
    from database import async_session
    from models import Job
    from pipeline.step_ia02_duplicates import _is_user_kept

    ts = int(time.time())
    print("\n── FTAG-36: _is_user_kept() ──")

    async with async_session() as session:
        # Job WITH user_kept
        kept = Job(
            filename=f"__v29_iuk_yes_{ts}.jpg",
            original_path=f"/tmp/__v29_iuk_yes_{ts}.jpg",
            source_inbox_path="/inbox", status="done",
            debug_key=f"V29-IUK-Y-{ts}",
            step_result={"IA-02": {"status": "skipped", "user_kept": True}},
        )
        # Job WITHOUT user_kept
        normal = Job(
            filename=f"__v29_iuk_no_{ts}.jpg",
            original_path=f"/tmp/__v29_iuk_no_{ts}.jpg",
            source_inbox_path="/inbox", status="done",
            debug_key=f"V29-IUK-N-{ts}",
            step_result={"IA-02": {"status": "ok"}},
        )
        # Job with empty step_result
        empty = Job(
            filename=f"__v29_iuk_empty_{ts}.jpg",
            original_path=f"/tmp/__v29_iuk_empty_{ts}.jpg",
            source_inbox_path="/inbox", status="done",
            debug_key=f"V29-IUK-E-{ts}",
            step_result={},
        )
        session.add_all([kept, normal, empty])
        await session.commit()

    check("FTAG-36a _is_user_kept(user_kept=True) → True",
          _is_user_kept(kept) is True,
          f"result={_is_user_kept(kept)}")

    check("FTAG-36b _is_user_kept(normal) → False",
          _is_user_kept(normal) is False,
          f"result={_is_user_kept(normal)}")

    check("FTAG-36c _is_user_kept(empty) → False",
          _is_user_kept(empty) is False,
          f"result={_is_user_kept(empty)}")


# ──────────────────────────────────────────────────────────────
# FTAG-37: quality_swap protection for user_kept jobs
# ──────────────────────────────────────────────────────────────
async def test_quality_swap_protection():
    """FTAG-37: user_kept jobs are NOT demoted by quality_swap (exact + pHash)."""
    from database import async_session
    from models import Job
    from pipeline.step_ia02_duplicates import _is_user_kept, _quality_score

    ts = int(time.time())
    print("\n── FTAG-37: quality_swap protection ──")

    async with async_session() as session:
        # Existing job: LOW quality but user_kept
        existing_kept = Job(
            filename=f"__v29_qs_existing_{ts}.jpg",
            original_path=f"/tmp/__v29_qs_existing_{ts}.jpg",
            source_inbox_path="/inbox", status="done",
            target_path=f"/library/photos/__v29_qs_existing_{ts}.jpg",
            debug_key=f"V29-QS-EX-{ts}",
            file_hash=f"v29qs_{ts}",
            phash="v29qs_00000001",
            step_result={
                "IA-01": {"file_type": "JPEG", "file_size": 1000,
                          "width": 800, "height": 600},
                "IA-02": {"status": "skipped", "user_kept": True},
            },
        )
        # New job: HIGHER quality (bigger, more pixels)
        new_better = Job(
            filename=f"__v29_qs_new_{ts}.jpg",
            original_path=f"/tmp/__v29_qs_new_{ts}.jpg",
            source_inbox_path="/inbox", status="queued",
            debug_key=f"V29-QS-NEW-{ts}",
            file_hash=f"v29qs_{ts}",
            phash="v29qs_00000001",
            step_result={
                "IA-01": {"file_type": "JPEG", "file_size": 5000000,
                          "width": 4032, "height": 3024},
                "IA-02": {"status": "ok"},
            },
        )
        session.add_all([existing_kept, new_better])
        await session.commit()

    # Verify new has better quality score
    new_score = _quality_score(new_better)
    ex_score = _quality_score(existing_kept)
    check("FTAG-37a new has better quality score",
          new_score > ex_score,
          f"new={new_score} > existing={ex_score}")

    # Verify user_kept blocks swap
    check("FTAG-37b _is_user_kept(existing) is True",
          _is_user_kept(existing_kept) is True)

    # The actual code guard: _quality_score(job) > _quality_score(existing) AND NOT _is_user_kept(existing)
    would_swap = _quality_score(new_better) > _quality_score(existing_kept) and not _is_user_kept(existing_kept)
    check("FTAG-37c quality_swap blocked by user_kept",
          would_swap is False,
          f"would_swap={would_swap}")

    # Verify that without user_kept it WOULD swap
    existing_kept.step_result["IA-02"]["user_kept"] = False
    would_swap_without = _quality_score(new_better) > _quality_score(existing_kept) and not _is_user_kept(existing_kept)
    check("FTAG-37d without user_kept, swap WOULD happen",
          would_swap_without is True,
          f"would_swap={would_swap_without}")

    # Restore for cleanup
    existing_kept.step_result["IA-02"]["user_kept"] = True
...
async def main():
    ...
    await test_user_kept_flag_keep()
    await test_user_kept_flag_not_duplicate()
    await test_is_user_kept()
    await test_quality_swap_protection()
